...
@section('content')
    <div class="space-y-6">
        <div>
            <h2 class="text-lg font-semibold text-slate-900 dark:text-white">{{ __('Repair Payments') }}</h2>
            <p class="text-sm text-slate-500">{{ __('Track deposits and final payments per invoice.') }}</p>
        </div>
    ...
    <script>
        const i18n = {
            unableToLoadPayments: @json(__('Unable to load payments.')),
            noPaymentsFound: @json(__('No payments found.')),
        };
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            async function loadPayments() {
                ...
                if (!response.ok) {
                    rows.innerHTML = '<tr><td class="px-4 py-6 text-center text-sm text-slate-500" colspan="6">' + i18n.unableToLoadPayments + '</td></tr>';
                    return;
                }
                ...
                rows.innerHTML = list.map(function (payment) {
                    ...
                    `;
                }).join('') || ('<tr><td class="px-4 py-6 text-center text-sm text-slate-500" colspan="6">' + i18n.noPaymentsFound + '</td></tr>');
            }
