...
@section('content')
        ...
        <div class="grid gap-6 {{ auth()->user()?->hasPermission('create_product_master') ? 'lg:grid-cols-[1.2fr_2fr]' : '' }}">
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                ...
                <form id="master-create-form" class="mt-4 space-y-4">
                    <div>
                        <label class="text-sm font-semibold text-slate-700 dark:text-slate-200" for="master-type">{{ __('Attribute') }}</label>
                    ...
                    <div>
                        <label class="text-sm font-semibold text-slate-700 dark:text-slate-200" for="master-value">{{ __('Value') }}</label>
                        <input id="master-value" name="value" type="text" placeholder="{{ __('e.g. 256GB') }}" class="mt-2 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700 focus:border-primary-500 focus:ring-primary-500 dark:border-slate-800 dark:bg-slate-900/60 dark:text-slate-200" />
                        <p id="master-value-error" class="mt-2 text-xs text-danger-600"></p>
                    </div>
                    <button class="inline-flex h-10 items-center rounded-xl bg-primary-600 px-4 text-sm font-semibold text-white shadow-sm">{{ __('Add Option') }}</button>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                <div class="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <div class="relative">
                        <input id="master-search" type="text" placeholder="{{ __('Search values') }}" class="h-10 w-60 rounded-xl border border-slate-200 bg-slate-50 px-3 text-sm text-slate-700 placeholder:text-slate-400 focus:border-primary-500 focus:ring-primary-500 dark:border-slate-800 dark:bg-slate-900/60 dark:text-slate-200" />
    ...
    <div id="edit-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40 px-4">
        <div class="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl dark:bg-slate-900">
            <div class="flex items-center justify-between">
                <h3 class="text-lg font-semibold text-slate-900 dark:text-white">{{ __('Edit Option') }}</h3>
            ...
            <form id="edit-modal-form" class="mt-4 space-y-4">
                ...
                <div>
                    <label class="text-sm font-semibold text-slate-700 dark:text-slate-200">{{ __('Type') }}</label>
                ...
                <div>
                    <label class="text-sm font-semibold text-slate-700 dark:text-slate-200">{{ __('Value') }}</label>
                ...
                <button class="inline-flex h-10 w-full items-center justify-center rounded-xl bg-primary-600 px-4 text-sm font-semibold text-white shadow-sm">{{ __('Update') }}</button>
    ...
    <div id="delete-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40 px-4">
        <div class="w-full max-w-sm rounded-2xl bg-white p-6 text-center shadow-xl dark:bg-slate-900">
            ...
            <h3 class="text-lg font-semibold text-slate-900 dark:text-white">{{ __('Delete option?') }}</h3>
            <p class="mt-2 text-sm text-slate-500 dark:text-slate-300">{{ __('This action cannot be undone.') }}</p>
            <p id="delete-form-error" class="mt-3 text-xs text-danger-600"></p>
            <div class="mt-5 flex items-center justify-center gap-3">
                <button id="delete-cancel" type="button" class="h-10 rounded-xl border border-slate-200 px-4 text-sm font-semibold text-slate-600 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-200">{{ __('Cancel') }}</button>
                <button id="delete-confirm" type="button" class="h-10 rounded-xl bg-rose-600 px-4 text-sm font-semibold text-white hover:bg-rose-700">{{ __('Delete') }}</button>
            </div>
        </div>
    </div>
