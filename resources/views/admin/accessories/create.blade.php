...
@section('title', __('Create Accessory'))
@section('page-title', __('Create Accessory'))
...
@section('content')
    <div class="grid gap-6 lg:grid-cols-[2fr_1fr]">
        <form id="accessory-create-form" enctype="multipart/form-data" class="space-y-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <div>
                <h2 class="text-lg font-semibold text-slate-900 dark:text-white">{{ __('Accessory Details') }}</h2>
                <p class="text-sm text-slate-500">{{ __('Add a new accessory or repair part for the catalog.') }}</p>
            </div>

            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    <label class="text-sm font-semibold text-slate-700 dark:text-slate-200" for="name">{{ __('Name') }}</label>
                    <input id="name" name="name" type="text" placeholder="{{ __('Charging Port') }}" class="mt-2 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700 focus:border-primary-500 focus:ring-primary-500 dark:border-slate-800 dark:bg-slate-900/60 dark:text-slate-200" />
                </div>
                <div>
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="4" placeholder="{{ __('Short details about this accessory') }}" class="mt-2 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700 focus:border-primary-500 focus:ring-primary-500 dark:border-slate-800 dark:bg-slate-900/60 dark:text-slate-200"></textarea>
            </div>

            <div class="flex items-center gap-3">
                <button class="inline-flex h-10 items-center rounded-xl bg-primary-600 px-4 text-sm font-semibold text-white shadow-sm">{{ __('Save Accessory') }}</button>
        ...
        <div class="space-y-6">
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900" x-data="{ preview: null }">
                ...
                <div class="mt-4 flex h-40 items-center justify-center rounded-xl border border-dashed border-slate-300 bg-slate-50 text-xs text-slate-500 dark:border-slate-700 dark:bg-slate-900/60">
                    <template x-if="preview">
                        <img :src="preview" alt="{{ __('Preview') }}" class="h-32 w-32 rounded-xl object-cover" />
                    </template>
                    <template x-if="!preview">
            ...
            <div class="rounded-2xl border border-slate-200 bg-slate-50 p-5 text-xs text-slate-500 dark:border-slate-800 dark:bg-slate-950">
                <p class="font-semibold text-slate-700 dark:text-slate-200">{{ __('Validation rules') }}</p>
                <p class="mt-2">{{ __('Only IPHONE and SAMSUNG brands are allowed. Warranty must match dropdown values.') }}</p>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        document.getElementById('accessory-create-form').addEventListener('submit', async function (event) {
            ...
            try {
                ...
                if (response.ok) {
                    if (window.adminSwalStore) {
                        window.adminSwalStore({
                            icon: 'success',
                            title: '{{ __('Accessory created') }}',
                            text: '{{ __('Accessory created successfully.') }}',
                            confirmButtonColor: '#2563eb',
                        });
                    } else if (window.adminToastStore) {
                        window.adminToastStore({ type: 'success', message: '{{ __('Accessory created successfully.') }}' });
                    }
                ...
                document.getElementById('accessory-form-error').textContent = errorData.message || '{{ __('Unable to save accessory.') }}';
                if (window.adminSwalError) {
                    window.adminSwalError('{{ __('Create failed') }}', errorData.message || '{{ __('Unable to save accessory.') }}');
                } else if (window.adminToast) {
                    window.adminToast(errorData.message || '{{ __('Unable to save accessory.') }}', { type: 'error' });
                }
            } catch (error) {
                document.getElementById('accessory-form-error').textContent = '{{ __('Unable to save accessory.') }}';
                if (window.adminSwalError) {
                    window.adminSwalError('{{ __('Create failed') }}', '{{ __('Unable to save accessory.') }}');
                } else if (window.adminToast) {
                    window.adminToast('{{ __('Unable to save accessory.') }}', { type: 'error' });
                }
            }
        });
    </script>
