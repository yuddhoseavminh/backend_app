...
@section('page-title', __('Repair Details'))
    ...
    <div class="space-y-6">
        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <div class="flex flex-wrap items-start justify-between gap-4">
                <div>
                    <h2 id="repair-title" class="text-xl font-semibold text-slate-900 dark:text-white">{{ __('Repair') }} #{{ $repairId }}</h2>
                    <p id="repair-subtitle" class="mt-1 text-sm text-slate-500">{{ __('Loading repair details...') }}</p>
                </div>
            ...
            <div class="mt-6 space-y-6">
                <div class="repair-panel" data-panel="intake">
                    <form id="intake-form" class="grid gap-4 lg:grid-cols-2">
                        <div>
                            <label class="text-sm font-semibold text-slate-700 dark:text-slate-200" for="imei_serial">{{ __('IMEI / Serial') }}</label>
                        ...
                        <div>
                            <label class="text-sm font-semibold text-slate-700 dark:text-slate-200" for="intake_photos">{{ __('Intake photos (comma separated)') }}</label>
                        ...
                        <div class="lg:col-span-2">
                            <label class="text-sm font-semibold text-slate-700 dark:text-slate-200" for="condition_checklist">{{ __('Condition checklist (comma separated)') }}</label>
                        ...
                        <div class="lg:col-span-2">
                            <label class="text-sm font-semibold text-slate-700 dark:text-slate-200" for="intake_notes">{{ __('Notes') }}</label>
                        ...
                        <div>
                            <button class="inline-flex h-10 items-center rounded-xl bg-primary-600 px-4 text-sm font-semibold text-white" type="submit">{{ __('Save intake') }}</button>
                ...
                <div class="repair-panel hidden" data-panel="diagnostic">
                    <form id="diagnostic-form" class="grid gap-4 lg:grid-cols-2">
                        <div class="lg:col-span-2">
                            <label class="text-sm font-semibold text-slate-700 dark:text-slate-200" for="problem_description">{{ __('Problem description') }}</label>
                        ...
                        <div>
                            <label class="text-sm font-semibold text-slate-700 dark:text-slate-200" for="parts_required">{{ __('Parts required (comma separated)') }}</label>
                        ...
                        <div>
                            <label class="text-sm font-semibold text-slate-700 dark:text-slate-200" for="labor_cost">{{ __('Labor cost') }}</label>
                        ...
                        <div class="lg:col-span-2">
                            <label class="text-sm font-semibold text-slate-700 dark:text-slate-200" for="diagnostic_notes">{{ __('Diagnostic notes') }}</label>
                        ...
                        <div>
                            <button class="inline-flex h-10 items-center rounded-xl bg-primary-600 px-4 text-sm font-semibold text-white" type="submit">{{ __('Save diagnostic') }}</button>
                ...
                <div class="repair-panel hidden" data-panel="quotation">
                    <div class="grid gap-6 lg:grid-cols-2">
                        <form id="quotation-form" class="grid gap-4">
                            <div>
                                <label class="text-sm font-semibold text-slate-700 dark:text-slate-200" for="quotation_parts">{{ __('Parts cost') }}</label>
                            ...
                            <div>
                                <label class="text-sm font-semibold text-slate-700 dark:text-slate-200" for="quotation_labor">{{ __('Labor cost') }}</label>
                            ...
                            <div>
                                <button class="inline-flex h-10 items-center rounded-xl bg-primary-600 px-4 text-sm font-semibold text-white" type="submit">{{ __('Save quotation') }}</button>
                            ...
                            <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-xs text-slate-500 dark:border-slate-800 dark:bg-slate-950">
                                <p class="font-semibold text-slate-700 dark:text-slate-200">{{ __('Current quote status') }}</p>
                        ...
                        <div class="rounded-2xl border border-slate-200 bg-slate-50 p-5 text-sm text-slate-600 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-300">
                            <p class="text-xs font-semibold uppercase tracking-widest text-slate-400">{{ __('Invoice') }}</p>
                            <p id="invoice-number" class="mt-3 text-lg font-semibold text-slate-900 dark:text-white">--</p>
                            <p id="invoice-total" class="text-sm text-slate-500">{{ __('Total:') }} --</p>
                ...
                <div class="repair-panel hidden" data-panel="chat">
                    <div class="grid gap-6 lg:grid-cols-[2fr_1fr]">
                        ...
                        <form id="chat-form" class="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-300">
                            <label class="text-xs font-semibold uppercase tracking-widest text-slate-400">{{ __('New message') }}</label>
                            ...
                            <button class="mt-3 inline-flex h-9 w-full items-center justify-center rounded-xl bg-primary-600 px-4 text-xs font-semibold text-white" type="submit">{{ __('Send') }}</button>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            async function loadTechnicians() {
                ...
                select.innerHTML = '<option value="">{{ __('Assign technician') }}</option>' + list.map(function (tech) {
            ...
            async function loadStatusTimeline() {
                ...
                container.innerHTML = list.map(function (log) {
                    ...
                    return '<div class="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-600 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300">' +
                        ...
                        '</div>';
                }).join('') || '<p class="text-sm text-slate-500">{{ __('No status updates yet.') }}</p>';
            }
